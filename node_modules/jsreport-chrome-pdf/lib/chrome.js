...
module.exports = function (reporter, definition) {
  ...
  reporter.initializeListeners.add(definition.name, () => {
    ...
    reporter.documentStore.collection('templates').beforeInsertListeners.add(definition.name, (doc) => {
      if (doc.chrome) {
        sanitizeNumberValue(doc.chrome, 'scale', 'chrome.scale')
      }

      if (doc.chromeImage) {
        sanitizeNumberValue(doc.chromeImage, 'quality', 'chromeImage.quality')
        sanitizeNumberValue(doc.chromeImage, 'clipX', 'chromeImage.clipX')
        sanitizeNumberValue(doc.chromeImage, 'clipY', 'chromeImage.clipY')
        sanitizeNumberValue(doc.chromeImage, 'clipWidth', 'chromeImage.clipWidth')
        sanitizeNumberValue(doc.chromeImage, 'clipHeight', 'chromeImage.clipHeight')
      }
    })

    reporter.documentStore.collection('templates').beforeUpdateListeners.add(definition.name, (query, update) => {
      if (update.$set && update.$set.chrome) {
        sanitizeNumberValue(update.$set.chrome, 'scale', 'chrome.scale', true)
      }

      if (update.$set && update.$set.chromeImage) {
        sanitizeNumberValue(update.$set.chromeImage, 'quality', 'chromeImage.quality', true)
        sanitizeNumberValue(update.$set.chromeImage, 'clipX', 'chromeImage.clipX', true)
        sanitizeNumberValue(update.$set.chromeImage, 'clipY', 'chromeImage.clipY', true)
        sanitizeNumberValue(update.$set.chromeImage, 'clipWidth', 'chromeImage.clipWidth', true)
        sanitizeNumberValue(update.$set.chromeImage, 'clipHeight', 'chromeImage.clipHeight', true)
      }
    })
  })
...
function sanitizeNumberValue (parent, prop, name, update = false) {
  let value
  ...
  if (value != null) {
    ...
    if (isNaN(value)) {
      throw new Error(`Can not ${update ? 'update' : 'insert'} template because "${name}" property should be a valid number`)
    }
