...
module.exports = class JobProcessor {
  ...
  async processOne (schedule, task) {
    ...
    try {
      ...
      if (count === 1) {
        return this.execute(schedule, task)
      }
    } catch (e) {
  ...
  async execute (schedule, task, executeNow) {
    const insertTaskIfNotExists = async () => {
      if (!task) {
        ...
        const doc = await this.documentStore.collection('tasks').insert(task)
        task._id = doc._id
      } else {
        this.currentlyRunningTasks.push(task)
      }
    ...
    try {
      ...
      const templates = await this.documentStore.collection('templates').find({shortid: schedule.templateShortid})
      if (templates.length > 0) {
        await this.executionHandler(schedule, task)
        this.logger.debug('Processing schedule ' + schedule.name + ' succeeded.')
        await this.documentStore.collection('tasks').update({ _id: task._id }, {
          $set: {
            state: 'success',
